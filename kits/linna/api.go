...
func (c *ApiConfiguration) Check(log *zap.Logger) error {
	if c.SSLCertificate != "" && c.SSLPrivateKey == "" {
		log.Fatal("SSL configuration invalid, specify both socket.ssl_certificate and socket.ssl_private_key", zap.String("param", "socket.ssl_certificate"))
	}
	if c.SSLCertificate == "" && c.SSLPrivateKey != "" {
		log.Fatal("SSL configuration invalid, specify both socket.ssl_certificate and socket.ssl_private_key", zap.String("param", "socket.ssl_private_key"))
	}

	if c.SSLCertificate != "" && c.SSLPrivateKey != "" {
		certPEMBlock, err := ioutil.ReadFile(c.SSLCertificate)
		if err != nil {
			log.Fatal("Error loading SSL certificate cert file", zap.Error(err))
		}

		keyPEMBlock, err := ioutil.ReadFile(c.SSLPrivateKey)
		if err != nil {
			log.Fatal("Error loading SSL certificate key file", zap.Error(err))
		}
		cert, err := tls.X509KeyPair(certPEMBlock, keyPEMBlock)
		if err != nil {
			log.Fatal("Error loading SSL certificate", zap.Error(err))
		}

		log.Info("SSL mode enabled")
		c.CertPEMBlock = certPEMBlock
		c.KeyPEMBlock = keyPEMBlock
		c.TLSCert = []tls.Certificate{cert}
	}
	return nil
}
