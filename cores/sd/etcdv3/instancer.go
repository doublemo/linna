...
func NewInstancer(c Client, prefix string, logger *zap.Logger) (*Instancer, error) {
	...
	if err == nil {
		logger.Info("NewInstancer", zap.String("prefix", s.prefix), zap.Int("instances", len(instances)))
	} else {
		logger.Error("NewInstancer", zap.String("prefix", s.prefix), zap.Error(err))
	}
...
func (s *Instancer) loop() {
	...
	for {
		...
		case <-ch:
			instances, err := s.client.GetEntries(s.prefix)
			if err != nil {
				s.logger.Error("failed to retrieve entries", zap.Error(err))
