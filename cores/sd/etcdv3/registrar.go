...
func NewRegistrar(client Client, service Service, logger *zap.Logger) *Registrar {
	return &Registrar{
		...
		service: service,
		logger: logger.With(
			zapcore.Field{Key: "key", Type: zapcore.StringType, String: service.Key},
			zapcore.Field{Key: "value", Type: zapcore.StringType, String: service.Value},
		),
	}
}
...
func (r *Registrar) Register() {
	if err := r.client.Register(r.service); err != nil {
		r.logger.Error("register sevice failed", zap.String("key", r.service.Key), zap.String("value", r.service.Value), zap.Error(err))
		return
	}
	if r.service.TTL != nil {
		r.logger.Info("register action", zap.String("key", r.service.Key), zap.String("value", r.service.Value), zap.Int64("lease", r.client.LeaseID()))
	} else {
		r.logger.Info("register action", zap.String("key", r.service.Key), zap.String("value", r.service.Value))
	}
}
...
func (r *Registrar) Deregister() {
	if err := r.client.Deregister(r.service); err != nil {
		r.logger.Error("deregister sevice failed", zap.String("key", r.service.Key), zap.String("value", r.service.Value), zap.Error(err))
	} else {
		r.logger.Info("deregister action", zap.String("key", r.service.Key), zap.String("value", r.service.Value))
	}
